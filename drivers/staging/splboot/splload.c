...
#define IMAGE_SPL_ADDR      0x12c00000 //RAM starts at 12c00000 for aria
                             ...
                             void *data)
{
	...
	if(!spl_image_stored){
		printk(KERN_EMERG "splboot - Need store image.\n");
		return 0;
	}
	printk(KERN_EMERG "splboot - Starting new spl\n");
	...
	printk(KERN_INFO "splboot - spladdr=%p s_spladdr=%08x\n", spladdr, s_spladdr);
...
static int __init splboot_init(void)
{
	...
	if(!splboot_dir) {
		printk("splboot - Couldn't make /proc/splboot dir");		
		return -1;
	}
printk("splboot - proc directory made\n");
...
printk("splboot - mddiaddr=%p\n", mddiaddr);
printk("splboot - bootimgaddr=%p\n", bootimgaddr);
	...
	spl_image_stored = 0;
	printk(KERN_INFO "splboot - Module sucessfully loaded, SE umad?, address : " "%d", IMAGE_SPL_ADDR);
	return 0;
fail:
	splboot_free();
	return -ENOMEM;
...
//Were closing module here, will not be used frequently  ..
static void __exit splboot_exit(void)
{
	printk(KERN_INFO "splboot - Module sucessfully unloaded, good bye!\n");
	splboot_free();
}
